...
subroutine tphysbc (ztodt,               &
    ...
    integer :: mon_rank                 ! calender month indices
    real(r8), parameter :: cool_prfl(30)=(/0.00000000 , 0.00000000 , 0.00000000 &
    ...
    !*** MOD START: Import Cooling Profile 
    ...
    do mon_rank = 1,12
        if (calday < day_rank(mon_rank)+1) then
            exit
        end if
    end do

    !Get JJA to execute cooling
    if ((mon_rank>=6) .and. (mon_rank<=8)) then
        do i = 1,ncol 
            stat_lat=state%lat(i)*180.0/3.1415926
            stat_lon=state%lon(i)*180.0/3.1415926
            if (stat_lat >= 7.5 .and. stat_lat <= 17.5 &
                .and. stat_lon >= 120.0 .and. stat_lon<= 140.0) then    ! TP mask = True
                ! Import the cooling force
                ptend%s(i,:) = ptend%s(i,:) - cool_prfl*1.0*cpair
                
                ! Below to output debug info
                if ((calday-floor(calday))<0.01) then 
                    if ((abs(stat_lon-130.0)<0.25).and. &
                    (abs(stat_lat-10.0)<0.25)) then
                        write(iulog,"(A30,F8.2,A5,F6.2,A5,F6.2)") "COOLING= TRUE Calday:",calday,&
