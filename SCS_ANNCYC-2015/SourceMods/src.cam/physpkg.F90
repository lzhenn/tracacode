...
subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_out )
    ...
    type(file_desc_t) ::  fh_rfmask
    !allocate(rfmask(pcols,begchunk:endchunk,12))   ! Rainfall Mask flag
    logical :: found=.false.
    
    integer :: nm
    character(len=256) :: bnd_rfmask_loc   ! filepath of topo file on local disk

    !*** MOD END: define field



    !*** MOD START: infld RF mask
    bnd_rfmask_loc ="/HOME/sysu_hjkx_ys/WORKSPACE/L_Zealot/cesm/B/B2000_f09_CAM5PM_SCS_ANNCYC/data/clm_g_mon_pre.nc"
    !bnd_rfmask_loc ="/users/yangsong3/CESM/input/atm/cam/sst/sst_HadOIBl_bc_0.9x1.25_clim_c040926.nc"
    call cam_pio_openfile(fh_rfmask, bnd_rfmask_loc, 0)
    !rfmask(:,:,:) = 0.0
    
    do nm = 1, 12
        call infld('RFMASK', fh_rfmask, 'lon', 'lat', 1, pcols, begchunk, endchunk, &
        rfmask(:,:,nm), found, grid_map='PHYS', timelevel=nm)
    end do
    !*** MOD END: infld RF mask 
...
subroutine tphysbc (ztodt,               &
    ...
    integer :: jj                 ! dayrank indices
    integer :: ii                 ! column indices
    ...
    call t_stopf('convect_deep_tend')
        ...
        !*** MOD START: Intensifying convection heating by 10%
        ...
        do ii = 1,ncol 
            ...
            if (stat_lat >= -30.0 .and. stat_lat <= 45.0 &
                .and. stat_lon >= 100.0 .and. stat_lon<= 160.0) then 

                if (rfmask(ii,lchnk,jj) > 0.5) then    ! RF mask = True
                    ! Amplify the heat force by 10%, less than 0 excluded
                    do k = 1,pver
                        if (ptend%s(ii,k) > 0) then
                            ptend%s(ii,k) = 1.1*ptend%s(ii,k)
                        end if
                    end do
                    ...
                    if ((calday-floor(calday))<0.01) then 
                        ! EA Test
                        if ((abs(stat_lon-120.0)<0.5).and. &
                        (abs(stat_lat-30.0)<0.5)) then
                            write(iulog,"(A40,I4)") "EA Region, RFMASK= TRUE, MON_RANK:",jj
                            exit
                        end if
                        ! Austrilia Test
                        if ((abs(stat_lon-130.0)<0.5).and. &
                        (abs(stat_lat-(-10.0))<0.5)) then
                            write(iulog,"(A40,I4)") "Aus Region, RFMASK= TRUE, MON_RANK:",jj
                            exit
                        end if
        ...
        !*** MOD END: Intensifying convection heating by 10%
