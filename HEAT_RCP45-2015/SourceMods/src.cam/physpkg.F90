...
subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_out )
    ...
    integer, parameter :: fin = 100
    integer :: status = 0
    character(len=100) :: filename="/users/yangsong3/L_Zealot/F/data/model_post/rcp45.PTENDT_DIFF_tropical_mon.bin"
    ...
    open(unit=fin, file=filename, form="unformatted", access="sequential", convert="big_endian")
    read(fin) heatf%lat
    read(fin) heatf%lon
    read(fin) heatf%value
    close(fin)
    write(iulog,*) "HAVE READ ",filename
...
subroutine tphysbc (ztodt,               &
    ...
    integer  i,j,k,m                             ! Longitude, level, constituent indices
    integer :: ixcldice, ixcldliq              ! constituent indices for cloud liquid and ice water.
   ...
   do i = 1,ncol 
        ...
        if ((stat_lat > -30.0) .and. &
        (stat_lat < 30.0)) then
            do pos_lat = 1,nlat
                if (abs(heatf%lat(pos_lat)-stat_lat)<0.01) then
                    exit
                end if
            end do
            do pos_lon = 1,nlon
                if (abs(heatf%lon(pos_lon)-stat_lon)<0.01) then
                    exit
                end if
            end do
            ! Inject the heat force
            ptend%s(i,:) = ptend%s(i,:) +1.0*heatf%value(:,pos_lat,pos_lon,j)
            
            ! Below to output debug info
            if ((calday-floor(calday))<0.01) then 
                if ((abs(stat_lon-120.0)<1.0).and. &
                (abs(stat_lat-25.0)<1.0)) then
                    write(iulog,"(A30,F8.2,A5,F6.2,A5,F6.2,A5,F6.2,A5,F6.2,A12,F8.5)") "USE_HEATF= TRUE Calday:",calday,&
                    "Lat:",stat_lat,"Latf:",heatf%lat(pos_lat),"Lon:",stat_lon,"Lonf:",heatf%lon(pos_lon),"Heatf(lv20)=",&
                    heatf%value(20,pos_lat,pos_lon,j)
                    exit
                end if
            end if
        end if
