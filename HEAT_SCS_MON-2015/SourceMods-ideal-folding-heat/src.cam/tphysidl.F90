...
!
   ...
   real(r8) :: zero(pcols)
! ****L_Zealot MOD START: Define Variables ****
   real(r8) :: stat_lat, stat_lon              ! lat and lon of a certain column
   real(r8) :: latS, latN, lonW, lonE, latC, lonC          ! Heating region boundary
   real(r8) :: bd_folding                      ! Regional heat folding
   real(r8), parameter :: heat_prfl(30)=(/0.00000000 , 0.00000000 , 0.00000000 &
...
! ****L_Zealot MOD END: Define Variables ****
!
!-----------------------------------------------------------------------
!
! ****L_Zealot MOD START: Initialize Variables ****
   latS = 0._r8
   latN = 15.0
   lonW = 110.0
   lonE = 150.0
   ...
   latC = (latS+latN)*0.5
   lonC = (lonW+lonE)*0.5
! ****L_Zealot MOD END: Initialize Variables ****
   idlflag = 1
...
! **** L_Zealot MOD START: Import Heating Profile ****
      do i = 1,ncol 
        stat_lat=state%lat(i)*180.0/3.1415926
        stat_lon=state%lon(i)*180.0/3.1415926
        if (stat_lat >= latS .and. stat_lat <= latN &
            .and. stat_lon >= lonW .and. stat_lon<= lonE) then    
            
            !bd_folding = 1 - (abs(stat_lat-latC)/abs(latN-latC))*(abs(stat_lon-lonC)/abs(lonE-lonC))
            bd_folding = (1 - abs(stat_lat-latC)/abs(latN-latC))*(1 - abs(stat_lon-lonC)/abs(lonE-lonC))
            ! Import the heating force
            ! 4K/day, in one day, (max about 50pgm response)
            !if ( nstep .lt. 49) then 
            ! profile ---- center 1K/day
            ! 2.0     ---- amplified factor
            ! cpair   ---- 1000.4
            ! bd_folding - folding factor

                ptend%s(i,:) = ptend%s(i,:) + heat_prfl*4.0*cpair*bd_folding 
            !end if
            ! Below to output debug info
            if (mod(nstep,48) .eq. 0) then 
                if ((abs(stat_lon-130.0)<0.5).and. &
                (abs(stat_lat-10.0)<0.5)) then
                    write(iulog,"(A30,F8.2,A5,F6.2,A5,F6.2)") "HEATING= TRUE nstep:",nstep,&
                    "Lat:",stat_lat,"Lon:",stat_lon
                end if
            end if
            ! Upper to output debug info
