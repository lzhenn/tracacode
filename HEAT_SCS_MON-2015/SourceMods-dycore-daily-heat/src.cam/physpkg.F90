...
!****MOD START: CALL ADIABATIC HEATING****
          call t_startf('adiabheat')
          call adiabheat(ztodt, phys_state(c), phys_tend(c))
          call t_stopf('adiabheat')
!****MOD END: CALL ADIABATIC HEATING****
...
! ****MOD START: ADIABHEAT SUBROUTINE****
subroutine adiabheat(ztodt, state, tend)
   use shr_kind_mod,       only: r8 => shr_kind_r8
...
!   end do
   ...
   call physics_ptend_init(ptend, state%psetcols, 'adiabheat', ls=.true., lu=.true., lv=.true.)

   
    
    ! Day one peturbation, insert heating profile
    do i = 1,ncol 
        stat_lat=state%lat(i)*180.0/3.1415926
        stat_lon=state%lon(i)*180.0/3.1415926
        if (stat_lat >= 0.0 .and. stat_lat <= 15.0 &
            .and. stat_lon >= 110.0 .and. stat_lon<= 155.0) then    
            
            ! Import the heating force
            ! 4K/day, in one day, (max about 50pgm response)
            !if ( nstep .lt. 49) then 
                ptend%s(i,:) = ptend%s(i,:) + heat_prfl*1.0*cpair 
            !end if
...
!*** MOD END: Import Cooling Profile
...
! update the state and total physics tendency
   ...
   call check_energy_chng(state, tend, "adiabheat", nstep, ztodt, zero, zero, zero, zero)

end subroutine adiabheat 



! ****MOD END: ADIABHEAT SUBROUTINE****
