...
nc_path='/home/lzhenn/array74/data/archive/shu/2018091200_noluzon/wrfout_d01_2018-09-12_00:00:00'
...
simdir='/home/lzhenn/array74/data/archive/njord/'
#esms=['2018091200_njord','2018091200_pgw']
esms=['2018091200','2018091200pgw_nudging','2018091200pgw']
esmnames=['REAL_WORLD','FUTURE_WARMING','NO_LUZON_LANDFALL']
line_libs=['b-.*','r-^', 'g-^']

ax.plot(df_obv['HKO_LON'], df_obv['HKO_LAT'],
            color='black', marker='o', linewidth=2, markersize=5, 
            transform=ccrs.Geodetic(), label='HKO best')


for idx, case in enumerate(esms):
    print(f'Read {case}...')
    sim_fn=f'{simdir}/{case}'
    df_sim=pd.read_csv(
            f'{sim_fn}/tc_track.csv', parse_dates=['time'],index_col=['time'])

    dateparse = lambda x: datetime.datetime.strptime(x, '%Y%m%d%H%M%S')

    ax.plot(df_sim['lon'], df_sim['lat'],
        line_libs[idx], linewidth=1, markersize=3, 
        transform=ccrs.Geodetic(), 
        label=esmnames[idx])

ax.pcolormesh(lons, lats, lsmask, 
              zorder=0, alpha=0.5, 
...
plt.savefig('../fig/trck.mangkhut.'+FIG_FMT, dpi=300, bbox_inches='tight')
plt.close('all')
#plt.show()
