...
def main():
    # File paths 
    cn_map_file='/home/metctm1/array/workspace/spellcaster-local/data/CN-border-La.dat'
    sta_meta_file='/home/metctm1/array/workspace/spellcaster-local/data/station/SURF_CLI_CHN_PRE_MUT_HOMO_STATION.xls'
    fcst_dir='/home/metctm1/array/data/1905-GD-LSTM-FORECAST/'
    out_dir='../fig/'


    # Get in files
    fcst_fig='fcst.t2m.png'
   
    #S2S data
    ds_s2s = xr.open_dataset(fcst_dir+'CFSv2.T2m.20210424.202105.nc')
...
#    plt.rcParams['font.sans-serif']=['SimHei'] 
    ...
    for idx, row in sta_df.iterrows():
        ...
        var=fcst_var1.sel(LAT=lat_sta, LON=lon_sta, method='nearest').values
        # 6-level PS forecast
        if var>2.0:
    ...
    ax.scatter( lon_dic['p2'], lat_dic['p2'],marker='.', color='darkred', 
            s=40,zorder=0, transform=ccrs.PlateCarree(), label='>2.0℃ Sta_Num:'+str(len(lon_dic['p2'])))
    # +
    ax.scatter( lon_dic['p1'], lat_dic['p1'],marker='.', color='red', 
            s=15,zorder=1, transform=ccrs.PlateCarree(), label='1.0~2.0℃ Sta_Num:'+str(len(lon_dic['p1'])))
    # +o
    ax.scatter( lon_dic['p0'], lat_dic['p0'],marker='.', color='gold', 
            s=8,zorder=2, transform=ccrs.PlateCarree(), label='0.0~1.0℃ Sta_Num:'+str(len(lon_dic['p0'])))
    # -o
    ax.scatter( lon_dic['p-0'], lat_dic['p-0'],marker='.', color='skyblue', 
            s=10,zorder=2, transform=ccrs.PlateCarree(), label='-1.0~0.0℃ Sta_Num:'+str(len(lon_dic['p-0'])))
    # -
    ax.scatter( lon_dic['p-1'], lat_dic['p-1'],marker='.', color='blue', 
            s=20,zorder=1, transform=ccrs.PlateCarree(), label='-2.0~-1.0℃ Sta_Num:'+str(len(lon_dic['p-1'])))
    # --
    ax.scatter( lon_dic['p-2'], lat_dic['p-2'],marker='.', color='darkblue', 
            s=50,zorder=0, transform=ccrs.PlateCarree(), label='<-2.0℃ Sta_Num:'+str(len(lon_dic['p-2'])))


    plt.legend(loc='best', fontsize=SMFONT)
    
    plt.title('CFSv2 Monthly Temperature Anomaly Forecast (Target: 2021-04, Init Date: 2021-03-24)')
