...
def main():
    ...
    ctrl_path='../data/1911-COAWST/joe_tc_ctrl/tc_pos_slp.txt'
    sen_path='../data/1911-COAWST/joe_tc_sen/tc_pos_slp.txt'

    df_ctrl=pd.read_csv(ctrl_path, sep='\s+')
    df_sen=pd.read_csv(sen_path, sep='\s+')
    
    fig, ax = plt.subplots()
    plt.plot(df_ctrl['slp'], label='CTRL', color='blue')
    plt.plot(df_sen['slp'], label='SEN', color='red')
    ...
    fig.tight_layout()
    plt.show()
    savefig('../fig/tc-develop-timeseries.png')
