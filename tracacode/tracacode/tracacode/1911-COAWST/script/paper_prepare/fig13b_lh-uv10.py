...
#    cases=["ERA5_C2008_dynlim",  "ERA5_TY2001_nolimit",  "ERA5_WRFROMS_add", "ERA5_WRF_add"]
    ...
    fig, ax=plt.subplots()
    fig.subplots_adjust(left=0.12, bottom=0.15, right=0.97, top=0.92, wspace=None, hspace=None) 
    ...
    for (dot_case, shade_color, line_color, case) in zip(dot_lib, shade_color_lib, line_lib, cases):
        ...
        lh_heat=bin_means*bin_counts*3600*27000*27000/10e16 # 1e16 J
    ...
    plt.ylabel('Total LH (10^16 Joules)',fontsize=MIDFONT)
    plt.xticks(fontsize=MIDFONT)
    plt.yticks(fontsize=MIDFONT)
      
    plt.title( '(b) Accumulated Total '+varname+' - 10m WindSpeed', fontsize=BIGFONT, loc='left')
    fig.set_size_inches(FIG_WIDTH, FIG_HEIGHT)
    fig.savefig('../../fig/paper/fig13b_acc_'+varname+'.pdf')
    #plt.show()
