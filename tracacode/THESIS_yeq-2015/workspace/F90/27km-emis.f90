...
!!!!!! souce(1:5)=(/'power','agriculture','industry','residential','transportation'/)
	...
	real::grid(4,cx,cy),grid_china_in(cx,cy),grid_china_out(cx,cy),pow_dens(18),ind_dens(18)

        real,allocatable,dimension(:,:,:,:,:)::dat_cmaq_over
        real,allocatable,dimension(:,:,:,:)::coe,dat_over_1,dat_over_2,dat_over_3!!!dat_over_1=power,dat_over_3=industry,dat_over_2=all of other
   ...
   !     open(102,file=''//trim(file_name)//'dat_pic.grd',status='old',form='unformatted',access='direct',recl=cx,swap)
	...
	irec=0
	 ...
	 do j=1,cy
         ...
         dat_over_2(i,j,jj,k)=dat_cmaq_over(i,j,2,jj,dat_m(k))*coe(2,dat_m(k),dat_w(k),dat_h(k))+dat_cmaq_over(i,j,4,jj,dat_m(k))*coe(4,dat_m(k),dat_w(k),dat_h(k))+dat_cmaq_over(i,j,5,jj,dat_m(k))*coe(5,dat_m(k),dat_w(k),dat_h(k))
	end do
        end do
        end do
        end do
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       status=nf_open(''//trim(file_name)//'egts.2010112800_27.ncf',nf_write,ncid)
         write(*,*)'get nc'
...
!           CO(i,j,k,ii)=dat_over_1(i,j,pic(kk),ii)*pow_dens(k)+dat_over_3(i,j,pic(kk),ii)*ind_dens(k)
          ...
          do i=1,cx
           do j=1,cy
            do ii=1,nt         
              CO(i,j,1,ii)=(dat_over_2(i,j,pic(kk),ii)+dat_over_1(i,j,pic(kk),ii)*pow_dens(1)+dat_over_3(i,j,pic(kk),ii)*ind_dens(1))*grid_china_in(i,j)+CO(i,j,1,ii)*grid_china_out(i,j)
