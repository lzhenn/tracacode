...
#    cases=["ERA5_C2008_dynlim",  "ERA5_TY2001_nolimit",  "ERA5_WRFROMS_add", "ERA5_WRF_add"]
    ...
    fig, ax=plt.subplots()
    fig.subplots_adjust(left=0.1, bottom=0.15, right=0.97, top=0.92, wspace=None, hspace=None) 
    ...
    for (dot_case, shade_color, line_color, case) in zip(dot_lib, shade_color_lib, line_lib, cases):
        ...
        # read raw input
        ds = salem.open_wrf_dataset(wrf_root+case+'/wrfout_d02')
        ...
        bin_stds, bin_edges, binnumber = stats.binned_statistic(ws_box_comp.flatten(),
                                var1_box_comp.flatten(), statistic='std', bins=nbins)

        x_pos=(bin_edges[0:-1]+bin_edges[1:])/2

        tck=interpolate.splrep(x_pos, bin_means, s=0)
        tck_std=interpolate.splrep(x_pos, bin_stds, s=0)

        x_spline = np.linspace(x_pos.min(), x_pos.max(), 300)  
        mean_smooth = interpolate.splev(x_spline, tck, der=0)
        std_smooth = interpolate.splev(x_spline, tck_std, der=0)

        # scatter
        #ax.plot(x_pos, lh_heat, linewidth=0.0, color=line_color, marker=dot_case, markersize=8)
        ax.plot(x_spline, mean_smooth, label=case, linewidth=2., color=line_color )
        ax.fill_between(x_spline, mean_smooth-std_smooth, mean_smooth+std_smooth, alpha=0.2, color=shade_color)
    plt.legend(loc='best', fontsize=MIDFONT)
    plt.xlabel('10m WindSpeed (m/s)',fontsize=MIDFONT)
    plt.ylabel('AKMS (m/s)',fontsize=MIDFONT)
    plt.xticks(fontsize=MIDFONT)
    plt.yticks(fontsize=MIDFONT)
      
    plt.title( '(b) AKMS - 10m WindSpeed', fontsize=BIGFONT, loc='left')
    fig.set_size_inches(FIG_WIDTH, FIG_HEIGHT)
    fig.savefig('../../fig/paper/fig11_b_'+varname+'.pdf')
    #plt.show()
