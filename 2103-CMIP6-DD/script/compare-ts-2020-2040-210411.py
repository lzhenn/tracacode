...
for i in range(0,24):
    ...
    fn_stream=subprocess.check_output('ls '+REF_DIR+'wrfout_d04_2020-0[6-9]-??_'+DIAG_HR+'*', shell=True).decode('utf-8')
    ...
    fn_stream=subprocess.check_output('ls '+SEN_DIR+'wrfout_d04_2045-0[6-9]-??_'+DIAG_HR+'*', shell=True).decode('utf-8')
    fn_list=fn_stream.split()
    wrf_list=[Dataset(itm) for itm in fn_list]
