...
! *** LZN MOD START ***
  public shff   ! The sensible heat forcing that will be prescribed into the model
  public lhff   ! The latent heat forcing that will be prescribed into the model
  public swnf   ! The shortwave net flux forcing that will be prescribed into the model
  public lwdf   ! The longwave net flux forcing that will be prescribed into the model
! *** LZN MOD STOP ***

  real(r8), allocatable:: landm(:,:)     ! land/ocean/sea ice flag
! *** LZN MOD START ***
  real(r8), allocatable:: shff(:,:,:)    ! Relaxed U flag
  real(r8), allocatable:: lhff(:,:,:)    ! Relaxed V flag
  real(r8), allocatable:: swnf(:,:,:)    ! Relaxed V flag
  real(r8), allocatable:: lwdf(:,:,:)    ! Relaxed V flag
! *** LZN MOD STOP ***
...
!-----------------------------------------------------------------------
    ...
    if(.not. (adiabatic .or. ideal_phys)) then
       ...
       ! *** LZN MOD START ***
       allocate (shff  (pcols,begchunk:endchunk,12)) ! 12 --- timeframe
       allocate (lhff  (pcols,begchunk:endchunk,12))
       allocate (swnf  (pcols,begchunk:endchunk,12))
       allocate (lwdf  (pcols,begchunk:endchunk,12))
       ! *** LZN MOD STOP ***
