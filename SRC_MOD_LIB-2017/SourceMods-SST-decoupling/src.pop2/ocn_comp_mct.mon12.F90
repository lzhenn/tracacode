...
! ***LZN: MOD START***
   real (r4), dimension(:,:,:,:), allocatable :: &
      TEMP_DATA,        &! temp array for SST restoring (nx_block, ny_block, km, dayinyear, max_blocks)
      WGT_DATA           ! coefficient array for SST restoring (nx_block, ny_block, km, max_blocks)
! ***LZN: MOD END***
...
! ***LZN: MOD START***
 subroutine ptempforcing_read
   ...
   integer (i4) :: dcpl_time_frames=12
   ...
   allocate(TEMP_DATA(nx_block, ny_block, dcpl_time_frames, max_blocks_clinic))
   allocate(WGT_DATA(nx_block, ny_block, dcpl_time_frames, max_blocks_clinic))
   WGT_DATA=1.0
   ...
   t_dim = construct_io_dim('time',dcpl_time_frames)

   io_temp = construct_io_field('TEMP_12',dim1=i_dim,dim2=j_dim,dim3=t_dim,&
   ...
   io_ce = construct_io_field('WGT_12', dim1=i_dim, dim2=j_dim,dim3=t_dim,&
             long_name='nudging coefficients', units='1',&
             grid_loc=' 3111', field_loc = field_loc_center,&
             field_type = field_type_scalar, r3d_array = WGT_DATA)
   call data_set(ptempf_file, 'define', io_ce)
   call data_set(ptempf_file, 'read', io_ce)
...
!   call destroy_io_field(ru_data)
 ...
 subroutine ptempforcing_fix
    ...
    do iblock = 1, nblocks_clinic
       TRACER(:,:,1,1,curtime,iblock) =  &
       (TEMP_DATA(:,:,imonth,iblock)*WGT_DATA(:,:,imonth, iblock)+ &
       TRACER(:,:,1,1,curtime,iblock)*(1-WGT_DATA(:,:,imonth, iblock)))
    end do
    !$OMP END PARALLEL DO
