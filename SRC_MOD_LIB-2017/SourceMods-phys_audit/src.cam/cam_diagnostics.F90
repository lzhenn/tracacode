...
! ===============================================================================
! LZN PHYS AUDIT
subroutine define_audit_fld(proc_str, proc_full_name)

   use cam_history,        only: addfld, phys_decomp

   character(len=*), intent(in) ::  proc_str  ! process string in BF/AF_SHRT 
   character(len=*), intent(in) ::  proc_full_name  ! process string in BF/AF_SHRT 

   call addfld ('T_'//proc_str,'K',pver, 'A','Air temp '//proc_full_name, phys_decomp)
   call addfld ('U_'//proc_str,'m/s',pver, 'A','Zonal wind '//proc_full_name, phys_decomp)
   call addfld ('V_'//proc_str,'m/s',pver, 'A','Meridional wind '//proc_full_name, phys_decomp)
   call addfld ('S_'//proc_str,'J/kg',pver, 'A','Dry static energy '//proc_full_name, phys_decomp)
   call addfld ('W_'//proc_str,'Pa/s',pver, 'A','Omega '//proc_full_name, phys_decomp)
   call addfld ('PDEL_'//proc_str,'Pa',pver, 'A','Layer thickness '//proc_full_name, phys_decomp)
   call addfld ('ZM_'//proc_str,'m',pver, 'A','Midpoint geop. h. '//proc_full_name, phys_decomp)
   call addfld ('Q_'//proc_str,'kg/kg',pver, 'A','Water vapor mixing ratio '//proc_full_name, phys_decomp)
   call addfld ('PS_'//proc_str,'Pa',1, 'A','Surf. pres. '//proc_full_name, phys_decomp)
   
   call addfld ('M_'//proc_str,'kg',pver, 'A','Gridbox air mass '//proc_full_name, phys_decomp)
   call addfld ('INTE_'//proc_str,'J',pver, 'A','Gridbox internal energy '//proc_full_name, phys_decomp)
   call addfld ('KNTE_'//proc_str,'J',pver, 'A','Gridbox kinetic energy '//proc_full_name, phys_decomp)
   call addfld ('LATE_'//proc_str,'J',pver, 'A','Gridbox unreleased latent heat '//proc_full_name, phys_decomp)
