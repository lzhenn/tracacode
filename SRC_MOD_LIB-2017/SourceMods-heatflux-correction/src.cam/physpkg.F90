...
subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_out )
    ...
    !*** LZN MOD START: Use module
    use ncdio_atm,          only: infld
    use pio,          only: file_desc_t
    use ioFileMod,        only: getfil
    use cam_pio_utils,    only: cam_pio_openfile
    use comsrf,           only: shff, lhff, swnf, lwdf 
    !*** LZN MOD END: Use module
    ...
    !*** LZN MOD START: define field
    type(file_desc_t) ::  fh_forc
    logical :: found=.false.
    integer :: nm                       ! time frame controller 
    character(len=256) :: forc_loc     ! filepath of the forcing file
    
    !*** LZN MOD END: define field
    ...
    do nm=1, 12 
        call infld('SHFLXF', fh_forc, 'lon', 'lat', 1, pcols,  begchunk, endchunk, &
             shff(:,:,nm), found, grid_map='PHYS', timelevel=nm)
        
        call infld('LHFLXF', fh_forc, 'lon', 'lat', 1, pcols,  begchunk, endchunk, &
             lhff(:,:,nm), found, grid_map='PHYS', timelevel=nm)
        
        call infld('FSNSF', fh_forc, 'lon', 'lat', 1, pcols,   begchunk, endchunk, &
             swnf(:,:,nm), found, grid_map='PHYS', timelevel=nm)
...
subroutine tphysac (ztodt,   cam_in,  &
    ...
    if (do_clubb_sgs) then
       ...
       call vertical_diffusion_tend (ztodt ,state ,cam_in%wsx, cam_in%wsy,   &
        ...
        !*** LZN MOD START: change shortwave and longwave to the model state
...
subroutine tphysbc (ztodt,               &
    ...
    real(r8) :: calday,  G0
    real(r8) :: stat_lat, stat_lon
    integer :: j                 ! column indices, time frame
    !*** LZN MOD END: Define variable
