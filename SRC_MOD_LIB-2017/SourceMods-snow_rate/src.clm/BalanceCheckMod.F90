...
!-----------------------------------------------------------------------
    ...
    ! *** MOD: ignore the grid within our snow-changed region ***    
    !do c = lbc, ubc    
    !   if ((day>=335.0 .and. day<=365.0) .or. (day>=1.0 .and. day<=59.0)) then
    !       if ((londeg(indexg)>=70.0) .and. (londeg(indexg)<=125.0) .and. (latdeg(indexg)>=35.0) .and. (latdeg(indexg)<=47.5)) then
    !           errh2o(c) = 0.0_r8
    !       end if
    !   end if
    !end do
