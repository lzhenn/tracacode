...
!-----------------------------------------------------------------------
   ...
   !*** MOD START: Add large-scale snowfall rate
   ...
   do ii = 1,ncol   ! cols in chunk 
        ...
        if ((calday>=335.0 .and. calday<=365.0) .or. (calday>=1.0 .and. calday<=59.0) &
        .and. stat_lat>=35.0 .and. stat_lat<=47.5 .and. stat_lon>=70.0 .and. stat_lon<=125.0) then
            cam_out%precsl(ii) = cam_out%precsl(ii)+0.1/90.0/86400.0
            cam_out%precl(ii) = cam_out%precl(ii)+0.1/90.0/86400.0

        ! Below to output debug info
            ! EA Test
            if ((calday>=1.0 .and. calday<=50.0) .and. (abs(stat_lon-80.0)<0.5) .and. (abs(stat_lat-40.0)<0.5)) then
                write(iulog,*) "MOD FLAG: Calday:",calday,"Lat:",stat_lat,"Lon:",stat_lon," precsl=",cam_out%precsl(ii)
            end if
        end if
    end do
   !*** MOD END: Add large-scale snowfall rate
