...
subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_out )
    ...
    integer :: status = 0
    character(len=100) :: filename="/users/yangsong3/L_Zealot/B/data/model_post/rcp45.PTENDT_DIFF_SCS_pos_mon.bin"
  ...
  !Below to get out which month we are in
   ...
   do i = 1,ncol 
        ...
        (stat_lat < 30.0)) then
            ...
            ptend%s(i,:) = ptend%s(i,:) +2.0*heatf%value(:,pos_lat,pos_lon,j)
