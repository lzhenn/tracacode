     ...
     do ib = 1, nband
        do fp = 1,num_nourbanp
           ...
           if (pwtgcell(p)>0._r8 .or. ityplun(l)==istice_mec) then
              ...
              ! *** MOD START: change fabd and fabi ***
              ! Here we come to the boreal summer-half year
              if ((month>=3) .and. (month <=9)) then 
                  if ((londeg(g)>=60.0) .and. (londeg(g)<=120.0) .and. (latdeg(g)>=0) .and. (latdeg(g)<=45)) then
                      cad(p,ib) = cad(p,ib)*1.2
                      cai(p,ib) = cai(p,ib)*1.2
                      if ((abs(londeg(g)-115.0)<1.3) .and.(abs(latdeg(g)-30.0)<1.0) .and. (day-1.0 < 0.5) ) then
                          write(iulog,*) "***MOD: lon=", londeg(g), "lat=", latdeg(g), "month=", month
                          write(iulog,*) "***MOD: cad=", cad(p,ib), "balanc_cab=", forc_solad(g,ib)*fabd(p,ib)
                      end if
                  end if
              end if
             
              ! *** MOD END: change fabd and fabi ***
              ...
              ! *** MOD START: change albgrd and albgri ***
              ! Here we come to the boreal summer-half year
              if ((month>=3) .and. (month <=9)) then 
                  if ((londeg(g)>=60.0) .and. (londeg(g)<=120.0) .and. (latdeg(g)>=0) .and. (latdeg(g)<=45)) then
                      absrad  = trd(p,ib)*(1._r8-albgrd(c,ib)*0.8) + tri(p,ib)*(1._r8-albgri(c,ib)*0.8)
                      if ((abs(londeg(g)-115.0)<1.3) .and.(abs(latdeg(g)-30.0)<1.0) .and. (day-1.0 < 0.5) ) then
                          write(iulog,*) "***MOD: lon=", londeg(g), "lat=", latdeg(g), "month=", month
                          write(iulog,*) "***MOD: absrad=", absrad, "balanc_absrad=", trd(p,ib)*(1._r8-albgrd(c,ib)) + tri(p,ib)*(1._r8-albgri(c,ib))
                      end if
                  end if
              end if
             
              ! *** MOD END: change albgrd and albgri ***
