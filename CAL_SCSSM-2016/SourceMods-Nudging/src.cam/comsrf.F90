...
! *** MOD START ***
  public urelax   ! The U field that will be relaxed into the model
  public vrelax   ! The V field that will be relaxed into the model
! *** MOD STOP ***

  real(r8), allocatable:: landm(:,:)     ! land/ocean/sea ice flag
! *** MOD START ***
  real(r8), allocatable:: urelax(:,:,:,:)    ! Relaxed U flag
  real(r8), allocatable:: vrelax(:,:,:,:)    ! Relaxed V flag
! *** MOD STOP ***
...
!-----------------------------------------------------------------------
    ...
    if(.not. (adiabatic .or. ideal_phys)) then
       ...
       ! *** MOD START ***
       allocate (urelax  (pcols,pver,begchunk:endchunk,46)) ! 46 --- timeframe
       allocate (vrelax  (pcols,pver,begchunk:endchunk,46))
       ! *** MOD STOP ***
