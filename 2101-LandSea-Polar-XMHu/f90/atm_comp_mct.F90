  ...
  subroutine atm_import_mct( x2a_a, cam_in )
    ...
    do c=begchunk,endchunk
       ...
       do i =1,ncols                                                               
          ...
          ! LZN MOD: Valid SST/TS
          if ((cam_in(c)%ocnfrac(i) .gt. 0.0) .and. (cam_in(c)%sst(i) .gt. 260.0)) then
              sst_rep=cam_in(c)%sst(i)
          else if ((cam_in(c)%ocnfrac(i) .gt. 0.0) .and. (cam_in(c)%sst(i) .lt. 260.0)) then
              cam_in(c)%sst(i)=sst_rep
              cam_in(c)%ts(i)=sst_rep
          end if
          ! LZN MOD: Valid SST/TS
